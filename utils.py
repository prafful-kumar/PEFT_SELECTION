...
def forward_selection(score_loader, model):
    """
    a forward pass on target dataset
    :params score_loader: the dataloader for scoring transferability
    :params model: the model for scoring transferability
    ...
    model.eval()
    with torch.no_grad():
        for i, (data, target) in enumerate(score_loader):
            if i == 0:
                targets.append(target)
                data = data.cuda()
                _features = model(data)
                features.append(_features)
                running_mean = torch.mean(_features, dim=0)
                running_var = torch.var(_features, dim=0)
            else:
                data = data.cuda()
                _features = model(data)
                batch_mean = torch.mean(_features, dim=0)
                batch_var = torch.var(_features, dim=0)
                running_mean = (1 - momentum) * batch_mean + momentum * running_mean
                running_var = (1 - momentum) * batch_var + momentum * running_var
                distances_squared = torch.sum((X - running_mean) ** 2, dim=1)
                radio = torch.sum(runnning_var, dim=1)
                condition = distances_squared <= radio
                features.append(_features)
